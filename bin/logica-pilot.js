...
async function cmdTool(tool, args) {
  ...
  for (const k of ['schema', 'fields']) if (typeof a[k] === 'string') { try { a[k] = JSON.parse(a[k]); } catch {} }
  for (const k of ['index', 'maxSteps', 'limit', 'amount', 'maxElements', 'timeout', 'concurrency']) {
