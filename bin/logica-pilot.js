...
async function cmdTool(tool, args) {
  ...
  for (const k of ['schema', 'fields', 'includePaths', 'excludePaths', 'notify', 'rows', 'location', 'params', 'steps', 'assertions', 'response', 'headers', 'expressions', 'entry', 'engine', 'input']) if (typeof a[k] === 'string') { try { a[k] = JSON.parse(a[k]); } catch {} }
